    ...
    projects: [
      {
        ...
        bullets: [
          "Migrated the LiDAR visualizer from Canvas 2D to React Three Fiber after dense warehouse maps exceeded Canvas rendering limits — GPU acceleration handled the spatial complexity Canvas physically couldn't sustain.",
          'Diagnosed a persistent 5 FPS frame rate problem as a bandwidth issue, not a rendering one — chose WebSocket with Float16 + Deflate compression over WebRTC after testing both. Bandwidth dropped from 1.5 MB to 100 KB per frame, throughput rose to 50+ FPS.',
          'Refactored the frontend from a page-based structure to feature-based modules after 1.5 years of growth made the page-based approach unmaintainable — logic and components were tangled across pages with duplication spreading with every new feature.',
          ...
          'Built a map merge workflow handling 3 independent coordinate systems (canvas origin, map origin, merge/rotate space) with custom transformation math — operators align, rotate, and translate maps visually; coordinates recalculate on save.',
          'Built an Immer-based undo-redo system using patch-based state diffing — roughly 90% reduction in memory usage vs. full-snapshot approach.',
          'Migrated the backend from Django with unstructured WebSocket scripts to an organized FastAPI architecture; reduced Docker image sizes via multi-stage builds.',
...
function ProjectCard({
  ...
  project: (typeof roles)[0]['projects'][0];
}) {
            ...
            {expanded && (
              <ul className="mb-3 flex flex-col gap-2">
                {project.bullets.map((b) => (
                  <li
                    key={b}
                    className="flex gap-2 text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed"
                  >
      ...
      <div className="max-w-5xl mx-auto">
        <Reveal>
          ...
          <p className="text-center text-sm text-zinc-500 dark:text-zinc-400 mb-14">
            3 years building real-time systems and robotics interfaces at
            Thedush.
          </p>
        </Reveal>
